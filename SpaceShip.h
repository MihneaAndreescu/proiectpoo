...
public:
    ...
    void prepDrawGasCircles() {
        ...
        if (m_movingForward) {
            addGasCircle(m_drawInfo.leftRocketVertexArray[1].position, length);
            addGasCircle(m_drawInfo.rightRocketVertexArray[1].position, length);
        }
        if (m_movingClockwise) {
            addGasCircle(m_drawInfo.leftRocketVertexArray[0].position, length);
        }
        if (m_movingCounterClockwise) {
            addGasCircle(m_drawInfo.rightRocketVertexArray[0].position, length);
        }
    }

    void addGasCircle(const sf::Vector2f& position, float length) {
